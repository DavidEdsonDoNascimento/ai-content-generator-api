...
import { createContentWorker } from './worker/content.worker.js';
 ...
 * mais a escrita final —, senão o `SIGTERM` mataria trabalho que estava a
...
async function start(): Promise<void> {
  ...
  const shutdown = async (signal: NodeJS.Signals): Promise<void> => {
    ...
    try {
      // `close()` para de puxar jobs novos e **aguarda** os ativos terminarem —
      // é o que evita abandonar um conteúdo em PROCESSING sem ninguém para
      // finalizá-lo. Só depois as conexões que ele usava são fechadas.
      await worker.close();
  ...
  logger.info(
    { concurrency: 5, aiDelayMs: env.AI_DELAY_MS, jobAttempts: env.JOB_ATTEMPTS },
